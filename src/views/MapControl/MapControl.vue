                ...
                <ul id="layerTree" class="layerTree"  v-show="colled">
                    <li v-for="i in layers" :key="i.name">
                        <a-col :span="22" :offset="2"> 
                            <input type="checkbox" :id="i.name" v-model="i.checked" @click="liClickMap($event)">
                            {{i.name}}
                        </a-col>
                    </li>
                </ul>
    ...
    methods: {
        ...
        liClickMap(e) {
            if(e.target.checked) {
                switch (e.target.id) {
                    case "OSM" : 
                        this.MapControl.getLayers().item(0).setVisible(true);
                        break;
                    case "高德地图" : 
                        this.MapControl.getLayers().item(1).setVisible(true);
                        break;
                    default : break;
                }
            } else {
                switch (e.target.id) {
                    case "OSM" : 
                        this.MapControl.getLayers().item(0).setVisible(false);
                        break;
                    case "高德地图" : 
                        this.MapControl.getLayers().item(1).setVisible(false);
                        break;
                    default : break;
                }
            }
