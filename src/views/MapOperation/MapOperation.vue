<template>
  <div class="MapOperation">
    ...
    <div id="MapOperation" :style="{ background: background }">
      ...
      >
        ...
        <ul id="layerTree" class="layerTree" v-show="colled">
          <li
            v-for="i in operation"
            :key="i.name"
            @click="liClickOperation(i.id)"
          >
    ...
    >
      <p>
        经度：<a-input placeholder="请输入经度" v-model="positionFrom.lon" />
      </p>
      <p>
        纬度：<a-input placeholder="请输入纬度" v-model="positionFrom.lat" />
      </p>
      <p>
        级别：<a-input placeholder="请输入级别" v-model="positionFrom.zoom" />
      </p>
    </a-modal>
    <!-- 提示框 -->
    <a-modal
    ...
    >
      <p>
        ...
        <a-select
          ...
          style="width: 120px"
          v-model="currentCoordinate"
        >
          <a-select-option
            :value="i.id"
            v-for="i in coordinateData"
            :key="i.id"
          >
            {{ i.name }}
          </a-select-option>
    ...
    <a-modal
      title="Title"
      :visible="visibleLayerControl"
      @ok="clickLayerControl"
      @cancel="visibleLayerControl = false"
    >
      <ul>
        <li v-for="item in layerControlData" :key="item.name">
          <p>图层: {{ item.name }}</p>
          <p>
            状态：<a-checkbox
...
export default {
  ...
  data() {
    return {
      ...
      visibleLayerControl: false,
      layerControlData: [
        { name: '高德矢量图层', checked: true, opacity: 1 },
        { name: 'OSM矢量图层', checked: true, opacity: 1 },
      ],
    }
  },
  watch: {
  ...
  methods: {
    ...
    // 地图图层探查
    MapLayerExploration() {
      var container = document.querySelector('#MapOperation')
      var radius = 75
      document.addEventListener('keydown', (evt) => {
        if (evt.which === 38) {
          radius = Math.min(radius + 5, 150)
          this.MapOperation.render()
          evt.preventDefault()
      ...
      // 在渲染层之前，进行一些剪辑
      this.gaodeLayer.on('prerender', (event) => {
        ...
        if (mousePosition) {
          ...
          var offset = ol.Render.getRenderPixel(event, [
      ...
      // 渲染层后，恢复画布上下文
      this.gaodeLayer.on('postrender', (event) => {
        var ctx = event.context
        ctx.restore()
      })
    },
    // 地图图层层级控制
    MapLayerLevel() {
      this.osmLayer.setZIndex(10) // 控制地图层级
    },
    // 地图图层组控制
    MapLayerGroup() {
      this.visibleLayerControl = true
    },

    // 确定地图定位
    clickPosition() {
      let view = this.MapOperation.getView()
      let py = ol.Proj.fromLonLat(
        [this.positionFrom.lon, this.positionFrom.lat],
        'EPSG:4326'
      )
      //平移地图设置中心点和级别
      view.setCenter(py)
    ...
    // 确定地图组控制显隐和透明度
    clickLayerControl() {
      this.visibleLayerControl = false
      this.gaodeLayer.setVisible(this.layerControlData[0].checked) // 控制地图显隐
      this.gaodeLayer.setOpacity(Number(this.layerControlData[0].opacity)) // 控制地图透明度
    },
  },
}
</script>
